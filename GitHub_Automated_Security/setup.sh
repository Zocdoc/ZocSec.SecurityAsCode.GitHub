...
then
	...
	targetkeyid=$(aws kms list-aliases --region us-east-1 --profile it-eng | grep  -A 1 -w alias/GitHub-Webhook3 | grep TargetKeyId | cut -d '"' -f 4)
	export KEYID=$targetkeyid

		if [ $targetkeyid ]; then
			echo 'GitHub-Webhook3 is Found. Will use keyID:' $targetkeyid
			...
			aws ssm put-parameter --name /GitHub-Webhook3/github_access_token --type String --value $encrypt_token --region $region --profile $profile
			aws ssm put-parameter --name /GitHub-Webhook3/github_webhook_secret  --type String --value $encrypt_secret --region $region --profile $profile
...
EOL
			...
			keyid=$(aws kms create-key --description github-webhook3 --region $region --profile $profile | grep KeyId | cut -d '"' -f 4)
			#echo $keyid
			export KEYID=$keyid
			aws kms create-alias --alias-name alias/GitHub-Webhook3 --target-key-id $keyid --region $region --profile $profile
