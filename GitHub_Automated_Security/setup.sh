...
then
	...
	export KEYID=$targetkeyid
		...
		if [ $targetkeyid ]; then
			echo 'GitHub-Webhook1 is Found. Will use keyID:' $targetkeyid
...
EOL
			...
			keyid=$(aws kms create-key --description github-webhook1 --region $region --profile $profile | grep KeyId | cut -d '"' -f 4)
			#echo $keyid
			export KEYID=$keyid
